...
// If DB is unavailable, block API routes early (keeps server up for frontend/health)
app.use('/api', (req, res, next) => {
  if (dbReady) return next();
  return res.status(503).json({
    status: 'error',
    message: 'Database unavailable. Please ensure MongoDB is running/reachable and restart the server.',
  });
});
...
const connectDB = async () => {
  try {
    ...
    dbReady = false;
    logger.warn('Continuing without database connection. API endpoints requiring DB will return 503 until MongoDB is reachable.');
  }
};
...
// Connect to DB in background (server stays up even if it fails)
connectDB();
