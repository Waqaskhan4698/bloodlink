...
router.post('/register', [
  ...
  try {
    ...
    // Validate age only for donors (must be 18+).
    // Note: age is derived from dateOfBirth (not from an "age" field).
    if (role === 'donor') {
    ...
    const parsedCoordinates = Array.isArray(providedCoords) && providedCoords.length === 2
      ? [Number(providedCoords[0]), Number(providedCoords[1])] // [lng, lat]
      : [0, 0];
