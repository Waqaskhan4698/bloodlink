...
import { FaEye, FaEyeSlash, FaHeart, FaSpinner, FaCheck, FaTimes, FaMapMarkerAlt } from 'react-icons/fa';
...
const RegisterPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    address: {
      ...
      zipCode: '',
      country: 'Pakistan',
      // GeoJSON-like structure expected by backend if provided
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2:
        const coords = formData.address?.coordinates?.coordinates;
        const hasCoords = Array.isArray(coords) && coords.length === 2 && Number.isFinite(coords[0]) && Number.isFinite(coords[1]);
        const mapUrl = hasCoords
          ? `https://staticmap.openstreetmap.de/staticmap.php?center=${coords[1]},${coords[0]}&zoom=14&size=800x300&markers=${coords[1]},${coords[0]},red-pushpin`
          : null;
              ...
              <div className="flex items-start justify-between gap-4 flex-col md:flex-row">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-neutral-600 mt-1">
                    Use your current location to help find nearby donors/requests.
                  </p>
                  {hasCoords && (
                    <p className="text-xs text-neutral-700 mt-2">
                      Saved coordinates: <span className="font-mono">{coords[1].toFixed(6)}, {coords[0].toFixed(6)}</span> (lat, lng)
                    </p>
                  )}
                </div>
                ...
                <button
                  type="button"
                  disabled={isRegistering || isLocating}
                  onClick={() => {
                  ...
                  className="px-4 py-2 rounded-lg bg-blood-600 text-white hover:bg-blood-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {isLocating ? <FaSpinner className="animate-spin" /> : <FaMapMarkerAlt />}
              ...
              {mapUrl && (
                <div className="mt-4">
                  <img
                    ...
                    className="w-full h-[220px] object-cover rounded-lg border border-neutral-200"
                    loading="lazy"
          ...
          <div className="space-y-6">
            ...
            {formData.role === 'donor' ? (
              ...
              <div className="p-4 bg-neutral-50 rounded-lg border border-neutral-200">
                <h4 className="font-medium text-neutral-900 mb-2">Registration Information</h4>
                <p className="text-sm text-neutral-600">
                  You are registering as a {formData.role === 'recipient' ? 'recipient' : 'system administrator'}. The donor eligibility conditions are only shown to blood donors.
                </p>
              </div>
            )}
          </div>
